...
struct Vector(T, ubyte N) if (N >= 2 && N <= 4) {
	...
	///
	Vector opUnary(string op)()  const  if (op == "+" || op == "-" || op == "~" || op == "!") {
		Vector ret = void;
		static foreach (i; 0..N) {
	...
	///
	Vector opBinary(string op, U)(U rhs)   const {
		Vector ret = void;
        static if (is(U : Vector)) {
            static foreach (i; 0..N) {
	...
	///
    Vector opBinaryRight(string op, U)(U lhs)   const {
        Vector ret = void;
        static if (is(U : Vector)) {
            static foreach (i; 0..N) {
    ...
    ///
	ref Vector opOpAssign(string op, U)(U rhs)   {
	...
	///
	U opCast(U)()   const if (isVector!U && U.size == size) {
		U ret = void;
		static foreach (i; 0..N) {
	...
	///
	T squaredDistanceTo(Vector v)   const {
	...
	static if (isFloatingPoint!T) {
		...
		T distanceTo(Vector other)   const {
...
Vector!(T, N) minByElem(T, int N)(const Vector!(T, N) a, const Vector!(T, N) b)   {
...
Vector!(T, N) maxByElem(T, int N)(const Vector!(T, N) a, const Vector!(T, N) b)   {
...
Vector!(T, N) absByElem(T, int N)(const Vector!(T, N) a)   {
    import liberty.math.functions : abs;
    Vector!(T, N) ret = void;
    static foreach (i; 0..N) {
	    ret.v[i] = abs(a.v[i]);
...
/// Returns dot product.
T dot(T, int N)(const Vector!(T, N) a, const Vector!(T, N) b)   {
    T sum = 0;
    static foreach (i; 0..N) {
	    sum += a.v[i] * b.v[i];
    }
...
Vector!(T, 3) cross(T)(const Vector!(T, 3) a, const Vector!(T, 3) b)   {
    return Vector!(T, 3)(a.y * b.z - a.z * b.y, a.z * b.x - a.x * b.z, a.x * b.y - a.y * b.x);
}
/// Returns 3D reflect.
Vector!(T, N) reflect(T, int N)(const Vector!(T, N) a, const Vector!(T, N) b)   {
