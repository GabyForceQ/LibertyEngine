...
**/
T degrees(T)(T x) pure nothrow @safe if (!isIntegral!T) {
  return x * (180 / PI);
}
...
**/
T radians(T)(T x) pure nothrow @safe if (!isIntegral!T) {
  return x * (PI / 180);
}
...
**/
S lerp(S, T)(S a, S b, T t) pure nothrow @safe if (is(typeof(t * b + (1 - t) * a) : S)) {
  return t * b + (1 - t) * a;
}
 ...
 * Clamp x in [min, max].
**/
T clamp(T)(T x, T min, T max) pure nothrow @safe {
  if (x < min) {
    return min;
  } else if (x > max) {
    return max;
 ...
 * Integer truncation.
**/
long ltrunc(real x) nothrow @safe {
 ...
 * Integer flooring.
**/
long lfloor(real x) nothrow @safe {
...
**/
T safeAsin(T)(T x) pure nothrow @safe {
...
**/
T safeAcos(T)(T x) pure nothrow @safe {
...
**/
T step(T)(T edge, T x) pure nothrow @safe {
	return (x < edge) ? 0 : 1;
}
 ...
 *
**/
T smoothStep(T)(T a, T b, T t) pure nothrow @safe {
	if (t <= a) {
		return 0;
	} else if (t >= b) {
		return 1;
...
**/
bool isPowerOf2(T)(T i) pure nothrow @safe if (isIntegral!T)
in {
	assert(i >= 0);
} do {
	return (i != 0) && ((i & (i - 1)) == 0);
 ...
 * Integer log2.
**/
int ilog2(T)(T i) nothrow @safe if (isIntegral!T)
in {
	assert(i > 0);
	assert(isPowerOf2(i));
} do {
...
**/
int nextPowerOf2(int i) pure nothrow @safe
out(result) {
 ...
 * Computes next power of 2.
**/
long nextPowerOf2(long i) pure nothrow @safe
out(result) {
	assert(isPowerOf2(result));
} do {
	long v = i - 1;
...
**/
T sinOverX(T)(T x) pure nothrow @safe {
	if (1 + x * x == 1) {
		return 1;
	}
	return sin(x) / x;
...
**/
T moduloWrap(T)(T a, T b) pure nothrow @safe if (isSigned!T)
in {
	assert(b > 0);
} do {
	T x;
...
**/
T inverseSqrt(T)(T x) pure nothrow @safe if (isFloatingPoint!T) {
	version(AsmX86) {
		static if (is(T == float)) {
			float result;
			asm pure nothrow @safe {
				movss XMM0, x;
				rsqrtss XMM0, XMM0;
				movss result, XMM0;
			}
